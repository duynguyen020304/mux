...
describe("KanbanService.getBoard", () => {
  test("returns default board for new project", async () => {
    const board = await service.getBoard(PROJECT_PATH);
    expect(board.projectPath).toBe(PROJECT_PATH);
    expect(board.columns.length).toBeGreaterThan(0);
    expect(board.tasks).toEqual({});
  });

  test("caches board after first read", async () => {
    const board1 = await service.getBoard(PROJECT_PATH);
    const board2 = await service.getBoard(PROJECT_PATH);
...
describe("KanbanService.updateTask", () => {
  test("updates task fields", async () => {
    ...
    const updated = await service.updateTask(PROJECT_PATH, taskId, {
      title: "Updated",
      priority: "urgent",
    });
  ...
  test("returns error for nonexistent task", async () => {
    const result = await service.updateTask(PROJECT_PATH, "nonexistent", { title: "X" });
...
describe("KanbanService.deleteTask", () => {
  test("deletes existing task", async () => {
    ...
    const deleted = await service.deleteTask(PROJECT_PATH, created.data.id);
  ...
  test("returns error for nonexistent task", async () => {
    const result = await service.deleteTask(PROJECT_PATH, "nonexistent");
    expect(result.success).toBe(false);
  });
  ...
  test("removes task from column order", async () => {
    ...
    await service.deleteTask(PROJECT_PATH, taskId);

    // Invalidate cache to re-read from disk
    service.invalidateCache(PROJECT_PATH);
    const board = await service.getBoard(PROJECT_PATH);
...
describe("KanbanService.moveTask", () => {
  test("moves task from backlog to in_progress", async () => {
    ...
    const moved = await service.moveTask(PROJECT_PATH, created.data.id, "in_progress");
  ...
  test("disallows invalid transition (done -> in_progress)", async () => {
    ...
    await service.moveTask(PROJECT_PATH, created.data.id, "in_progress");
    await service.moveTask(PROJECT_PATH, created.data.id, "in_review");
    await service.moveTask(PROJECT_PATH, created.data.id, "done");

    // done -> in_progress should fail
    const result = await service.moveTask(PROJECT_PATH, created.data.id, "in_progress");
    expect(result.success).toBe(false);
  });
  ...
  test("allows done -> backlog (reopen)", async () => {
    ...
    const result = await service.moveTask(PROJECT_PATH, created.data.id, "backlog");
    expect(result.success).toBe(true);
    if (!result.success) return;
    expect(result.data.status).toBe("backlog");
  });
  ...
  test("moves task to correct column order", async () => {
    ...
    await service.moveTask(PROJECT_PATH, taskId, "in_progress");
...
describe("KanbanService.reorderTasks", () => {
  test("reorders tasks within a column", async () => {
    ...
    const result = await service.reorderTasks(PROJECT_PATH, "backlog", newOrder);
    expect(result.success).toBe(true);
  ...
  test("returns error for nonexistent task in order", async () => {
    const result = await service.reorderTasks(PROJECT_PATH, "backlog", ["nonexistent"]);
    expect(result.success).toBe(false);
  });
});
...
describe("KanbanService.archiveTask", () => {
  test("archives a task", async () => {
    ...
    const result = await service.archiveTask(PROJECT_PATH, created.data.id, true);
    expect(result.success).toBe(true);
    if (!result.success) return;
  ...
  test("unarchives a task back to backlog", async () => {
    ...
    await service.archiveTask(PROJECT_PATH, created.data.id, true);
    const result = await service.archiveTask(PROJECT_PATH, created.data.id, false);
    expect(result.success).toBe(true);
    if (!result.success) return;
    expect(result.data.status).toBe("backlog");
...
describe("KanbanService.updateColumn", () => {
  test("updates column title", async () => {
    const result = await service.updateColumn(PROJECT_PATH, "backlog", { title: "Todo" });
    expect(result.success).toBe(true);
    if (!result.success) return;
  ...
  test("updates column wipLimit", async () => {
    const result = await service.updateColumn(PROJECT_PATH, "backlog", { wipLimit: 5 });
    expect(result.success).toBe(true);
    if (!result.success) return;
  ...
  test("clears wipLimit when set to null", async () => {
    await service.updateColumn(PROJECT_PATH, "backlog", { wipLimit: 5 });
    const result = await service.updateColumn(PROJECT_PATH, "backlog", { wipLimit: null });
    expect(result.success).toBe(true);
    if (!result.success) return;
  ...
  test("returns error for nonexistent column", async () => {
    const result = await service.updateColumn(PROJECT_PATH, "nonexistent", { title: "X" });
    expect(result.success).toBe(false);
  });
});
...
describe("KanbanService.promoteQueuedTask", () => {
  test("returns null when no queued tasks", async () => {
    const result = await service.promoteQueuedTask(PROJECT_PATH);
  ...
  test("promotes oldest queued task to in_progress", async () => {
    ...
    const promoted = await service.promoteQueuedTask(PROJECT_PATH);
...
describe("KanbanService.reorderColumns", () => {
  test("reorders columns", async () => {
    ...
    const result = await service.reorderColumns(PROJECT_PATH, reversed);
    expect(result.success).toBe(true);

    service.invalidateCache(PROJECT_PATH);
    const updated = await service.getBoard(PROJECT_PATH);
...
describe("KanbanService.countRunningTasks", () => {
  test("returns 0 for empty board", async () => {
    const count = await service.countRunningTasks(PROJECT_PATH);
    expect(count).toBe(0);
  });
  ...
  test("counts in_progress tasks", async () => {
    ...
    await service.moveTask(PROJECT_PATH, t1.data.id, "in_progress");
